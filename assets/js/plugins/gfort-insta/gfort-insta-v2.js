...
(function ($) {
    ...
    $.fn.gfortInsta = function (options) {
        ...
        imagesWrapper += '<div class="imagesWrapper ' + columnsWrapper + ' ' + gridGutter + '">';
        el.wrapInner(imagesWrapper);

        $.ajax({
            type: 'GET',
            url: 'https://graph.instagram.com/me/media?fields=id,username&access_token=' + settings.accessToken,
            dataType: 'jsonp',
            cache: true,
            success: function (response) {

                if (settings.showText === 'yes') {
                    el.append('<h6 class="font-size-13"><a href="https://www.instagram.com/' + response.data[0].username + '" target="_blank" rel="noopener" class="gfort-insta-username">' + settings.text + ' @' + response.data[0].username + '</a>' + '</h6>');
                }
                ...
                for (imagesCount = 0; imagesCount < settings.count && imagesCount < response.data.length; imagesCount += 1) {
                    ...
                    $.ajax({
                        type: 'GET',
                        url: 'https://graph.instagram.com/' + response.data[imagesCount].id + '?fields=media_url,permalink,username&access_token=' + settings.accessToken,
                        dataType: 'jsonp',
                        cache: true,
                        success: function (response) {

                            imageItem = '<div class="gfort-grid-item ' + colXL + ' ' + colLG + ' ' + colMD + ' ' + colSM + ' ' + colXS + '"><a class="gfort-insta-item ' + settings.linkClass + '" href="' + response.permalink + '" target="_blank" rel="noopener"><img src="' + response.media_url + '" alt="Instagram Image" /></a></div>';
                            el.find('.imagesWrapper').append(imageItem);
